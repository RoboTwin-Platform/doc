...
First, create the `processed_data` and `training_data` folders in the `policy/pi05` directory:
...
If success, you will find the `${task_name}-${task_config}-${expert_data_num}` folder under `policy/pi05/processed_data`.
...
Before generating the LerobotDataset format data for pi05,please make sure you have enough disk space under the `~/.cache`.This is because generating the `lerobotdataset` will require a large amount of space.And the datasets will be writed into `$XDG_CACHE_HOME`,which default path is  `~/.cache`.If you don't have enough disk space under the `~/.cache` path, please use the following command to set a different cache directory with sufficient space:

```bash
export XDG_CACHE_HOME=/path/to/your/cache
```

Now, we can directly generate the LerobotDataset format data for pi05
...
In `src/openpi/training/config.py`, there is a dictionary called `_CONFIGS`. You can modify PI05 configurations I’ve written:
...
```bash
# ckpt_path like: policy/pi05/checkpoints/pi05_aloha_full_base/demo_clean/30000
bash eval.sh ${task_name} ${task_config} ${train_config_name} ${model_name} ${seed} ${gpu_id}
# bash eval.sh beat_block_hammer demo_clean pi05_aloha_full_base demo_clean 0 0
...
# bash eval.sh beat_block_hammer demo_randomized pi05_aloha_full_base demo_clean 0 0
```
